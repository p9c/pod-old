...
const (
	// minInFlightBlocks is the minimum number of blocks that should be in the request queue for headers-first mode before requesting more.
...
// startSync will choose the best peer among the available candidate peers to download/sync the blockchain from.  When syncing is already running, it simply returns.  It also examines the candidates for any which are no longer candidates and removes them as needed.
func (sm *SyncManager) startSync() {
	...
	if err != nil {
		Log.Errorf.Print("Unable to query for segwit soft-fork state: %v", err)
		return
	}
	...
	for peer, state := range sm.peerStates {
		...
		if segwitActive && !peer.IsWitnessEnabled() {
			Log.Debugf.Print("peer %v not witness enabled, skipping", peer)
			continue
		}
	...
	if bestPeer != nil {
		...
		if err != nil {
			Log.Errorf.Print("Failed to get block locator for the "+
				"latest block: %v", err)
			return
		}
		Log.Infof.Print("Syncing to block height %d from peer %v",
			bestPeer.LastBlock(), bestPeer.Addr())
		// When the current height is less than a known checkpoint we can use block headers to learn about which blocks comprise the chain up to the checkpoint and perform less validation for them.  This is possible since each header contains the hash of the previous header and a merkle root.
		// Therefore if we validate all of the received headers link together properly and the checkpoint hashes match, we can be sure the hashes for the blocks in between are accurate.  Further, once the full blocks are downloaded, the merkle root is computed and compared against the value in the header which proves the full block hasn't been tampered with.
		...
		if sm.nextCheckpoint != nil &&
			...
			Log.Infof.Print("Downloading headers for blocks %d to "+
		...
		Log.Warnf.Print("No sync peer candidates available")
	}
}
...
func (sm *SyncManager) isSyncCandidate(peer *peerpkg.Peer) bool {
	...
	if sm.chainParams == &chaincfg.RegressionNetParams {
		...
		segwitActive, err := sm.chain.IsDeploymentActive(chaincfg.DeploymentSegwit)
		if err != nil {
			Log.Errorf.Print("Unable to query for segwit "+
...
// handleNewPeerMsg deals with new peers that have signalled they may be considered as a sync peer (they have already successfully negotiated).  It also starts syncing if needed.  It is invoked from the syncHandler goroutine.
...
// handleDonePeerMsg deals with peers that have signalled they are done.  It removes the peer as a candidate for syncing and in the case where it was the current sync peer, attempts to select a new best peer to sync from.  It is invoked from the syncHandler goroutine.
func (sm *SyncManager) handleDonePeerMsg(peer *peerpkg.Peer) {
	state, exists := sm.peerStates[peer]
	if !exists {
		Log.Warnf.Print("Received done peer message for unknown peer %s", peer)
		return
	}
	// Remove the peer from the list of candidate peers.
	delete(sm.peerStates, peer)
	Log.Infof.Print("Lost peer %s", peer)
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	if _, exists = sm.rejectedTxns[*txHash]; exists {
		Log.Debugf.Print("Ignoring unsolicited previously rejected "+
	...
	// Remove transaction from request maps. Either the mempool/chain already knows about it and as such we shouldn't have any more instances of trying to fetch it, or we failed to insert and thus we'll retry next time we get an inv.
	...
	if err != nil {
		...
		if _, ok := err.(mempool.RuleError); ok {
			Log.Debugf.Print("Rejected transaction %v from %s: %v",
				txHash, peer, err)
		} else {
			Log.Errorf.Print("Failed to process transaction %v: %v",
				txHash, err)
		}
...
func (sm *SyncManager) handleBlockMsg(bmsg *blockMsg) {
	...
	state, exists := sm.peerStates[peer]
	if !exists {
		Log.Warnf.Print("Received block message from unknown peer %s", peer)
		return
	}
	...
	if blockchain.ShouldHaveSerializedBlockHeight(header) {
		...
		if err != nil {
			Log.Warnf.Print("Unable to extract height from "+
				"coinbase tx: %v", err)
		} else {
	...
	if err != nil {
		...
		if _, ok := err.(blockchain.RuleError); ok {
			Log.Infof.Print("Rejected block %v from %s: %v", blockHash,
				peer, err)
			Log.Infof.Print("height %d", bmsg.block.Height())
		} else {
			Log.Errorf.Print("Failed to process block %v: %v",
				blockHash, err)
		}
	...
	if isOrphan {
		...
		if blockchain.ShouldHaveSerializedBlockHeight(header) {
			...
			if err != nil {
				...
				Log.Debugf.Print("Extracted height of %v from "+
		...
		locator, err := sm.chain.LatestBlockLocator()
		if err != nil {
			Log.Warnf.Print("Failed to get block locator for the "+
				"latest block: %v", err)
		} else {
	...
	// This is headers-first mode and the block is a checkpoint.  When there is a next checkpoint, get the next round of headers by asking for headers starting from the block after this one up to the next checkpoint.
	...
	if sm.nextCheckpoint != nil {
		...
		if err != nil {
			Log.Warnf.Print("Failed to send getheaders message to "+
				"peer %s: %v", peer.Addr(), err)
			return
		}
		Log.Infof.Print("Downloading headers for blocks %d to %d from "+
			"peer %s", prevHeight+1, sm.nextCheckpoint.Height,
			sm.syncPeer.Addr())
		return
	}
	// This is headers-first mode, the block is a checkpoint, and there are no more checkpoints, so switch to normal mode by requesting blocks from the block after this one up to the end of the chain (zero hash).
	sm.headersFirstMode = false
	sm.headerList.Init()
	Log.Infof.Print("Reached the final checkpoint -- switching to normal mode")
	locator := blockchain.BlockLocator([]*chainhash.Hash{blockHash})
	err = peer.PushGetBlocksMsg(locator, &zeroHash)
	if err != nil {
		Log.Warnf.Print("Failed to send getblocks message to peer %s: %v",
			peer.Addr(), err)
		return
	}
}
...
func (sm *SyncManager) fetchHeaderBlocks() {
	...
	if sm.startHeader == nil {
		Log.Warnf.Print("fetchHeaderBlocks called with no start header")
		return
	}
	...
	for e := sm.startHeader; e != nil; e = e.Next() {
		...
		if err != nil {
			Log.Warnf.Print("Unexpected failure when checking for "+
		...
		if !haveInv {
			...
			// If we're fetching from a witness enabled peer post-fork, then ensure that we receive all the witness data in the blocks.
...
// handleHeadersMsg handles block header messages from all peers.  Headers are requested when performing a headers-first sync.
func (sm *SyncManager) handleHeadersMsg(hmsg *headersMsg) {
	peer := hmsg.peer
	_, exists := sm.peerStates[peer]
	if !exists {
		Log.Warnf.Print("Received headers message from unknown peer %s", peer)
		return
	}
	...
	if !sm.headersFirstMode {
		Log.Warnf.Print("Got %d unrequested headers from %s -- "+
	...
	for _, blockHeader := range msg.Headers {
		...
		if prevNodeEl == nil {
			Log.Warnf.Print("Header list does not contain a previous" +
		...
		if prevNode.hash.IsEqual(&blockHeader.PrevBlock) {
			...
			Log.Warnf.Print("Received block header that does not "+
		...
		if node.height == sm.nextCheckpoint.Height {
			if node.hash.IsEqual(sm.nextCheckpoint.Hash) {
				...
				Log.Infof.Print("Verified downloaded block "+
					"header against checkpoint at height "+
					"%d/hash %s", node.height, node.hash)
			} else {
				Log.Warnf.Print("Block header at height %d/hash "+
	...
	if receivedCheckpoint {
		...
		Log.Infof.Print("Received %v block headers: Fetching blocks",
			sm.headerList.Len())
...
// haveInventory returns whether or not the inventory represented by the passed inventory vector is known.  This includes checking all of the various places inventory can be when it is in different states such as blocks that are part of the main chain, on a side chain, in the orphan pool, and transactions that are in the memory pool (either the main pool or orphan pool).
func (sm *SyncManager) haveInventory(invVect *wire.InvVect) (bool, error) {
	...
	case wire.InvTypeTx:
		...
		// Check if the transaction exists from the point of view of the end of the main chain.  Note that this is only a best effort since it is expensive to check existence of every output and the only purpose of this check is to avoid downloading already known transactions.
...
func (sm *SyncManager) handleInvMsg(imsg *invMsg) {
	...
	state, exists := sm.peerStates[peer]
	if !exists {
		Log.Warnf.Print("Received inv message from unknown peer %s", peer)
		return
	}
	...
	// If this inv contains a block announcement, and this isn't coming from our current sync peer or we're current, then update the last announced block for this peer. We'll use this information later to update the heights of peers based on blocks we've accepted that they previously announced.
	...
	for i, iv := range invVects {
		...
		if iv.Type == wire.InvTypeBlock {
			...
			if sm.chain.IsKnownOrphan(&iv.Hash) {
				...
				locator, err := sm.chain.LatestBlockLocator()
				if err != nil {
					Log.Errorf.Print("PEER: Failed to get block "+
	...
	// Request as much as possible at once.  Anything that won't fit into request will be requested on the next inv message.
	numRequested := 0
	...
	for len(requestQueue) != 0 {
		...
		case wire.InvTypeTx:
			...
			if _, exists := sm.requestedTxns[iv.Hash]; !exists {
				...
				// If the peer is capable, request the txn including all witness data.
				if peer.IsWitnessEnabled() {
...
// limitMap is a helper function for maps that require a maximum limit by evicting a random transaction if adding a new value would cause it to overflow the maximum allowed.
...
// blockHandler is the main handler for the sync manager.  It must be run as a goroutine.  It processes block and inv messages in a separate goroutine from the peer handlers so the block (MsgBlock) messages are handled by a single thread without needing to lock memory data structures.  This is important because the sync manager controls which blocks are needed and how the fetching should proceed.
	...
	for {
		...
		case m := <-sm.msgChan:
			...
			case pauseMsg:
				...
				<-msg.unpause
			default:
				Log.Warnf.Print("Invalid message type in block "+
					"handler: %T", msg)
			}
		case <-sm.quit:
			break out
...
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockAccepted:
		...
		if !ok {
			Log.Warnf.Print("Chain accepted notification is not a block.")
			break
		}
...
func (sm *SyncManager) Stop() error {
	if atomic.AddInt32(&sm.shutdown, 1) != 1 {
		Log.Warnf.Print("Sync manager is already in the process of " +
			"shutting down")
		return nil
	}
	Log.Infof.Print("Sync manager shutting down")
	close(sm.quit)
	sm.wg.Wait()
	return nil
}
