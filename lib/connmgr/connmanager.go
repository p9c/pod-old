...
type Config struct {
	...
	// GetNewAddress is a way to get an address to make a network connection to.  If nil, no new connections will be made automatically.
...
func (cm *ConnManager) handleFailedConn(c *ConnReq) {
	...
	if c.Permanent {
		...
		Log.Debugf.Print("Retrying connection to %v in %v", c, d)
		...
		if cm.failedAttempts >= maxFailedAttempts {
			Log.Debugf.Print("Max failed connection attempts reached: [%d] "+
...
out:
		...
		case req := <-cm.requests:
			...
			case handleConnected:
				...
				if _, ok := pending[connReq.id]; !ok {
					...
					Log.Debugf.Print("Ignoring connection for "+
				...
				Log.Debugf.Print("Connected to %v", connReq)
			...
			case handleDisconnected:
				...
				// All internal state has been cleaned up, if this connection is being removed, we will make no further attempts with this request.
				if !msg.retry {
				...
				if uint32(len(conns)) < cm.cfg.TargetOutbound ||
					connReq.Permanent {
					connReq.updateState(ConnPending)
					Log.Debugf.Print("Reconnecting to %v",
						connReq)
			...
			case handleFailed:
				...
				Log.Debugf.Print("Failed to connect to %v: %v",
					connReq, msg.err)
				cm.handleFailedConn(connReq)
			}
		case <-cm.quit:
	...
	cm.wg.Done()
	Log.Trace.Print("Connection handler done")
}
...
func (cm *ConnManager) NewConnReq() {
	...
	// Submit a request of a pending connection attempt to the connection manager. By registering the id before the connection is even established, we'll be able to later cancel the connection via the
	// Remove method.
...
func (cm *ConnManager) Connect(c *ConnReq) {
	...
	if atomic.LoadUint64(&c.id) == 0 {
		atomic.StoreUint64(&c.id, atomic.AddUint64(&cm.connReqCount, 1))
		// Submit a request of a pending connection attempt to the connection manager. By registering the id before the connection is even established, we'll be able to later cancel the connection via the Remove method.
		done := make(chan struct{})
		select {
		case cm.requests <- registerPending{c, done}:
		case <-cm.quit:
	...
	Log.Debugf.Print("Attempting to connect to %v", c)
...
func (cm *ConnManager) listenHandler(listener net.Listener) {
	Log.Infof.Print("Server listening on %s", listener.Addr())
	for atomic.LoadInt32(&cm.stop) == 0 {
		conn, err := listener.Accept()
		if err != nil {
			// Only log the error if not forcibly shutting down.
			if atomic.LoadInt32(&cm.stop) == 0 {
				Log.Errorf.Print("Can't accept connection: %v", err)
			}
			continue
		}
	...
	cm.wg.Done()
	Log.Tracef.Print("Listener handler done for %s", listener.Addr())
}
...
func (cm *ConnManager) Start() {
	...
	Log.Trace.Print("Connection manager started")
	cm.wg.Add(1)
	go cm.connHandler()
...
func (cm *ConnManager) Stop() {
	if atomic.AddInt32(&cm.stop, 1) != 1 {
		Log.Warnf.Print("Connection manager already stopped")
		return
	}
	...
	close(cm.quit)
	Log.Trace.Print("Connection manager stopped")
}
