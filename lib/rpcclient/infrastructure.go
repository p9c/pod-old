...
func (c *Client) handleMessage(msg []byte) {
	...
	if err != nil {
		Log.Warnf.Print("Remote server sent invalid message: %v", err)
		return
	}
	...
	if in.ID == nil {
		...
		if ntfn == nil {
			Log.Warn.Print("Malformed notification: missing " +
				"method and parameters")
			return
		}
		if ntfn.Method == "" {
			Log.Warn.Print("Malformed notification: missing method")
			return
		}
		// params are not optional: nil isn't valid (but len == 0 is)
		if ntfn.Params == nil {
			Log.Warn.Print("Malformed notification: missing params")
			return
		}
		// Deliver the notification.
		Log.Tracef.Print("Received notification [%s]", in.Method)
	...
	if *in.ID < 0 || *in.ID != math.Trunc(*in.ID) {
		Log.Warn.Print("Malformed response: invalid identifier")
		return
	}
	if in.rawResponse == nil {
		Log.Warn.Print("Malformed response: missing result and error")
		return
	}
	id := uint64(*in.ID)
	Log.Tracef.Print("Received response for id %d (result %s)", id, in.Result)
	request := c.removeRequest(id)
	// Nothing more to do if there is no request associated with this reply.
	if request == nil || request.responseChan == nil {
		Log.Warnf.Print("Received unexpected reply: %s (id %d)", in.Result,
			id)
		return
	}
...
out:
	for {
		...
		if err != nil {
			...
			if c.shouldLogReadError(err) {
				Log.Errorf.Print("Websocket receive error from "+
	...
	c.Disconnect()
	c.wg.Done()
	Log.Tracef.Print("RPC client input handler done for %s", c.config.Host)
}
...
cleanup:
	...
	c.wg.Done()
	Log.Tracef.Print("RPC client output handler done for %s", c.config.Host)
}
...
func (c *Client) reregisterNtfns() error {
	...
	if stateCopy.notifyBlocks {
		Log.Debugf.Print("Reregistering [notifyblocks]")
	...
	if stateCopy.notifyNewTx || stateCopy.notifyNewTxVerbose {
		Log.Debugf.Print("Reregistering [notifynewtransactions] (verbose=%v)",
	...
	if nslen > 0 {
		...
		Log.Debugf.Print("Reregistering [notifyspent] outpoints: %v", outpoints)
	...
	if nrlen > 0 {
		...
		Log.Debugf.Print("Reregistering [notifyreceived] addresses: %v", addresses)
...
func (c *Client) resendRequests() {
	...
	if err := c.reregisterNtfns(); err != nil {
		Log.Warnf.Print("Unable to re-establish notification state: %v", err)
		c.Disconnect()
		return
	}
	// Since it's possible to block on send and more requests might be added by the caller while resending, make a copy of all of the requests that need to be resent now and work from the copy.  This allows the lock to be released quickly.
	c.requestLock.Lock()
	...
	for _, jReq := range resendReqs {
		...
		Log.Tracef.Print("Sending command [%s] with id %d", jReq.method,
			jReq.id)
...
out:
	...
	reconnect:
		for {
			...
			if err != nil {
				c.retryCount++
				Log.Infof.Print("Failed to connect to %s: %v",
					c.config.Host, err)
				...
				Log.Infof.Print("Retrying connection to %s in "+
					"%s", c.config.Host, scaledDuration)
				time.Sleep(scaledDuration)
				continue reconnect
			}
			Log.Infof.Print("Reestablished connection to RPC server %s",
				c.config.Host)
	...
	c.wg.Done()
	Log.Tracef.Print("RPC client reconnect handler done for %s", c.config.Host)
}
...
func (c *Client) handleSendPostMessage(details *sendPostDetails) {
	...
	Log.Tracef.Print("Sending command [%s] with id %d", jReq.method, jReq.id)
...
cleanup:
	...
	c.wg.Done()
	Log.Tracef.Print("RPC client send handler done for %s", c.config.Host)
}
...
func (c *Client) doDisconnect() bool {
	...
	Log.Tracef.Print("Disconnecting RPC client %s", c.config.Host)
	close(c.disconnect)
	if c.wsConn != nil {
		c.wsConn.Close()
	}
...
func (c *Client) doShutdown() bool {
	...
	Log.Tracef.Print("Shutting down RPC client %s", c.config.Host)
	close(c.shutdown)
	return true
}
...
func (c *Client) start() {
	Log.Tracef.Print("Starting RPC client %s", c.config.Host)
...
type ConnConfig struct {
	...
	Pass string
	// TLS enables transport layer security encryption. It is recommended to always use TLS if the RPC server supports it as otherwise your username and password is sent across the wire in cleartext.
	TLS bool
	// Certificates are the bytes for a PEM-encoded certificate chain used for the TLS connection.  It has no effect if the DisableTLS parameter
	// is true.
	Certificates []byte
	// Proxy specifies to connect through a SOCKS 5 proxy server.  It may be an empty string if a proxy is not required.
	Proxy string
	// ProxyUser is an optional username to use for the proxy server if it requires authentication.  It has no effect if the Proxy parameter is not set.
	ProxyUser string
	// ProxyPass is an optional password to use for the proxy server if it requires authentication.  It has no effect if the Proxy parameter is not set.
	ProxyPass string
	// DisableAutoReconnect specifies the client should not automatically try to reconnect to the server when it has been disconnected.
	DisableAutoReconnect bool
	// DisableConnectOnNew specifies that a websocket client connection should not be tried when creating the client with New.  Instead, the client is created and returned unconnected, and Connect must be called manually.
	DisableConnectOnNew bool
	// HTTPPostMode instructs the client to run using multiple independent connections issuing HTTP POST requests instead of using the default of websockets.  Websockets are generally preferred as some of the features of the client such notifications only work with websockets, however, not all servers support the websocket extensions, so this flag can be set to true to use basic HTTP POST requests instead.
	HTTPPostMode bool
	// EnableBCInfoHacks is an option provided to enable compatibility hacks when connecting to blockchain.info RPC server
...
func dial(config *ConnConfig) (*websocket.Conn, error) {
	...
	// Create a websocket dialer that will be used to make the connection. It is modified by the proxy setting below as needed.
...
func New(config *ConnConfig, ntfnHandlers *NotificationHandlers) (*Client, error) {
	...
	if start {
		Log.Infof.Print("Established connection to RPC server %s",
			config.Host)
