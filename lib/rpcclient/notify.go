...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case json.BlockConnectedNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid block connected "+
				"notification: %v", err)
			return
		}
	...
	case json.FilteredBlockConnectedNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid filtered block "+
	...
	case json.RecvTxNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid recvtx notification: %v",
				err)
			return
		}
	...
	case json.RedeemingTxNtfnMethod:
		...
		tx, block, err := parseChainTxNtfnParams(ntfn.Params)
		if err != nil {
			Log.Warnf.Print("Received invalid redeemingtx "+
				"notification: %v", err)
			return
		}
	...
	case json.RelevantTxAcceptedNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid relevanttxaccepted "+
				"notification: %v", err)
			return
		}
	...
	case json.RescanFinishedNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid rescanfinished "+
				"notification: %v", err)
			return
		}
	...
	case json.RescanProgressNtfnMethod:
		...
		hash, height, blkTime, err := parseRescanProgressParams(ntfn.Params)
		if err != nil {
			Log.Warnf.Print("Received invalid rescanprogress "+
				"notification: %v", err)
			return
		}
	...
	case json.TxAcceptedNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid tx accepted "+
				"notification: %v", err)
			return
		}
	...
	case json.TxAcceptedVerboseNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid tx accepted verbose "+
				"notification: %v", err)
			return
		}
	...
	case json.PodConnectedNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid pod connected "+
				"notification: %v", err)
			return
		}
	...
	case json.AccountBalanceNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid account balance "+
				"notification: %v", err)
			return
		}
	...
	case json.WalletLockStateNtfnMethod:
		...
		if err != nil {
			Log.Warnf.Print("Received invalid wallet lock state "+
				"notification: %v", err)
			return
		}
