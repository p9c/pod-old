...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	// Create a single coinbase transaction and add it to the harness chain's utxo set and set the harness chain height such that the coinbase will mature in the next block.  This ensures the txpool accepts transactions which spend immature coinbases that will become mature in the next block.
	numOutputs := uint32(1)
