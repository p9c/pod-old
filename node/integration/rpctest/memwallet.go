...
type memWallet struct {
	...
	currentHeight int32
	// addrs tracks all addresses belonging to the wallet. The addresses are indexed by their keypath from the hdRoot.
	addrs map[uint32]util.Address
	// utxos is the set of utxos spendable by the wallet.
	utxos map[wire.OutPoint]*utxo
	// reorgJournal is a map storing an undo entry for each new block received. Once a block is disconnected, the undo entry for the particular height is evaluated, thereby rewinding the effect of the disconnected block on the wallet's set of spendable utxos.
