...
type txPrioItem struct {
	...
	priority float64
	feePerKB int64
	// dependsOn holds a map of transaction hashes which this one depends on.  It will only be set when the transaction references other transactions in the source pool and hence must come after them in a block.
...
type BlockTemplate struct {
	...
	Block *wire.MsgBlock
	// Fees contains the amount of fees each transaction in the generated template pays in base units.  Since the first transaction is the coinbase, the first entry (offset 0) will contain the negative of the sum of the fees of all other transactions.
	Fees []int64
	...
	Height int32
	// ValidPayAddress indicates whether or not the template coinbase pays to an address or is redeemable by anyone.  See the documentation on NewBlockTemplate for details on which this can be useful to generate templates without a coinbase payment address.
	ValidPayAddress bool
	// WitnessCommitment is a commitment to the witness data (if any) within the block. This field will only be populted once segregated witness has been activated, and the block contains a transaction which has witness data.
...
// NewBlockTemplate returns a new block template that is ready to be solved using the transactions from the passed transaction source pool and a coinbase that either pays to the passed address if it is not nil, or a coinbase that is redeemable by anyone if the passed address is nil.  The nil address functionality is useful since there are cases such as the getblocktemplate RPC where external mining software is responsible for creating their own coinbase which will replace the one generated for the block template.  Thus the need to have configured address can be avoided. The transactions selected and included are prioritized according to several factors.  First, each transaction has a priority calculated based on its value, age of inputs, and size.
// Transactions which consist of larger amounts, older inputs, and small sizes have the highest priority.  Second, a fee per kilobyte is calculated for each transaction.  Transactions with a higher fee per kilobyte are preferred.  Finally, the block generation related policy settings are all taken into account.
// Transactions which only spend outputs from other transactions already in the block chain are immediately added to a priority queue which either prioritizes based on the priority (then fee per kilobyte) or the fee per kilobyte (then priority) depending on whether or not the BlockPrioritySize policy setting allots space for high-priority transactions.
// Transactions which spend outputs from other transactions in the source pool are added to a dependency map so they can be added to the priority queue once the transactions they depend on have been included. Once the high-priority area (if configured) has been filled with transactions, or the priority falls below what is considered high-priority, the priority queue is updated to prioritize by fees per kilobyte (then priority).
// When the fees per kilobyte drop below the TxMinFreeFee policy setting, the transaction will be skipped unless the BlockMinSize policy setting is nonzero, in which case the block will be filled with the low-fee/free transactions until the block size reaches that minimum size. Any transactions which would cause the block to exceed the BlockMaxSize policy setting, exceed the maximum allowed signature operations per block, or otherwise cause the block to be invalid are skipped.
...
mempoolLoop:
	for _, txDesc := range sourceTxns {
		...
		// Calculate the final transaction priority using the input value age sum as well as the adjusted transaction size.  The formula is: sum(inputValue * inputAge) / adjustedTxSize
	...
	for priorityQueue.Len() > 0 {
		...
		case !segwitActive && tx.HasWitness():
			continue
		// Otherwise, Keep track of if we've included a transaction with witness data or not. If so, then we'll need to include the witness commitment as the last output in the coinbase transaction.
		case segwitActive && !witnessIncluded && tx.HasWitness():
			// If we're about to include a transaction bearing witness data, then we'll also need to include a witness commitment in the coinbase transaction. Therefore, we account for the additional weight within the block with a model coinbase tx with a witness commitment.
			...
			// In order to accurately account for the weight addition due to this coinbase transaction, we'll add the difference of the transaction before and after the addition of the commitment to the block weight.
		...
		// Spend the transaction inputs in the block utxo view and add an entry for it to ensure any transactions which reference this one have it available as an input and can ensure they aren't double spending.
		spendTransaction(blockUtxos, tx, nextBlockHeight)
		// Add the transaction to the block, increment counters, and save the fees and signature operation counts to the block template.
	...
	// Now that the actual transactions have been selected, update the block weight for the real transaction count and coinbase value with the total fees accordingly.
	...
	if witnessIncluded {
		...
		// Next, obtain the merkle root of a tree which consists of the wtxid of all transactions in the block. The coinbase transaction will have a special wtxid of all zeroes.
	...
	// Finally, perform a full check on the created block against the chain consensus rules to ensure it properly connects to the current best chain with no issues.
...
func (g *BlkTmplGenerator) UpdateBlockTime(msgBlock *wire.MsgBlock) error {
	// The new timestamp is potentially adjusted to ensure it comes after the median time of the last several blocks per the chain consensus rules.
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A btcutil.Block should use saved in the state to avoid recalculating all of the other transaction hashes. block.Transactions[0].InvalidateCache()
