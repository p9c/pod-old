...
func loadConfig() (*Config, []string, error) {	// Default config.
	...
	if err != nil {if e, ok := err.(*flags.Error); ok && e.Type == flags.ErrHelp {fmt.Fprintln(os.Stderr, err)
	...
	if preCfg.ShowVersion {fmt.Println(appName, "version", version())
		os.Exit(0)
	}	// Show the available commands and exit if the associated flag was specified.

	if preCfg.ListCommands {ListCommands()
		os.Exit(0)
...
serverConfigPath = filepath.Join(SPVHomeDir, "sac.conf")
		} else {serverConfigPath = filepath.Join(NodeHomeDir, "pod.conf")
		}
...
fmt.Fprintf(os.Stderr, "Error creating a default config file: %v\n", err)
	...
	if err != nil {if _, ok := err.(*os.PathError); !ok {fmt.Fprintf(os.Stderr, "Error parsing config file: %v\n",
				err)
	...
	if err != nil {if e, ok := err.(*flags.Error); !ok || e.Type != flags.ErrHelp {fmt.Fprintln(os.Stderr, usageMessage)
		}
	...
	}	if cfg.SimNet {numNets++
	}	if numNets > 1 {str := "%s: The testnet and simnet params can't be used " +
	...
	if cfg.Wallet != "" && cfg.RPCCert == DefaultRPCCertFile {cfg.RPCCert = DefaultWalletCertFile
...
func createDefaultConfigFile(
	...
	if TLSSubmatches != nil {destString += fmt.Sprintf("TLS=%s\n", TLSSubmatches[1])
	}
